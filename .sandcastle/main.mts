/**
 * Sandcastle orchestrator for fast-questionnaire.
 *
 ...
 * holds a GitHub token, the app's master secret, or a Vercel token.
 *
 ...
 * account goes in the repository-root `.env`, which
 * `npm --prefix .sandcastle run sandcastle` loads into this process and nothing
 * copies into the microVM. Both files are git-ignored.
 *
  ...
  if (missing.length > 0) {
    throw new Error(
      `Incomplete Vercel credential, missing: ${missing.join(", ")}. @vercel/sandbox ` +
        ...
        `.env.example) — \`npm --prefix .sandcastle run sandcastle\` loads it on the host and never forwards ` +
...
/**
 * The gates every issue must clear. The app is managed with `uv` and tested with
 * `pytest` (issue #1). `sync --locked` fails when `uv.lock` is missing or out of
 * date with `pyproject.toml`, so it doubles as the "did you commit the lockfile"
 * check, and it installs what `pytest` then runs against.
 */
const GATES = [
  ["sync", "uv", ["sync", "--locked"]],
  ["tests", "uv", ["run", "pytest"]],
 ...
 * would build on whatever `main` looked like when the failed attempt
...
const succeeds = (command: string, args: string[], env?: NodeJS.ProcessEnv): boolean => {
  try {
    execFileSync(command, args, { stdio: "pipe", env });
    return true;
  } catch {
    return false;
  }
    ...
    hooks: {
      sandbox: {
        onSandboxReady: [
          ...
          // The app is Python managed with `uv`, and the gates are `uv`
          // commands. The node24 image is kept because it is the one this
          // harness has been proven against; `uv` fetches whatever Python
          // `pyproject.toml` asks for, so the image's own Python is irrelevant.
          {
            command:
...
/**
 * Everything that must be true before an agent's work is allowed onto main.
...
const rejections = (agentRun: AgentRun): string[] => {
  ...
  // The gates run code the agent wrote, on this machine. They get the host's
  // environment minus the credentials this process loaded for itself, so a test
  // cannot read the token that spends on your Vercel account.
  const gateEnv = { ...process.env };
  for (const key of [...VERCEL_KEYS, "CLAUDE_CODE_OAUTH_TOKEN"]) delete gateEnv[key];

  for (const [name, command, args] of GATES) {
    if (!succeeds(command, [...args], gateEnv)) {
      problems.push(`gate failed on host: ${name}`);
      // Nothing after a failed install can be trusted to mean anything.
      if (name === "sync") return problems;
    }
 ...
 * branches and moves `main`, so two of these at once would race.
...
const main = async () => {
  ...
  for (const [index, wave] of waves.entries()) {
    ...
    // Serialized: each of these checks out branches and moves main.
    const landed: number[] = [];
    for (const [position, agentRun] of agentRuns.entries()) {
      try {
        ...
        // is broken is this checkout — a `main` that no longer has the merge.
